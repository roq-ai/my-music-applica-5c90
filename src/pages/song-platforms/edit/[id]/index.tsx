...
import { getSongPlatformById, updateSongPlatformById } from 'apiSdk/song-platforms';
...
function SongPlatformEditPage() {
  ...
  const handleSubmit = async (values: SongPlatformInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSongPlatformById(id, values);
      mutate(updated);
      resetForm();

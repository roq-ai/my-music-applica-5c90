...
import { getSongPlatforms } from 'apiSdk/song-platforms';
...
function DashboardPage() {
  ...
  const songPlatformStat = useSWR<PaginatedInterface<SongPlatformInterface>>(
    () => `/song-platforms`,
    () => getSongPlatforms({ limit: 1, offset: 0 }),
  );
  ...
  const platformStat = useSWR<PaginatedInterface<PlatformInterface>>(
    () => `/platforms`,
    () => getPlatforms({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Song Platforms',
      result: songPlatformStat?.data?.totalCount,
      isLoading: songPlatformStat.isLoading,
    ...
    {
      title: 'Platforms',
      result: platformStat?.data?.totalCount,
      isLoading: platformStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
